...
function zip() {
	...
	if ( opts.hasOwnProperty( 'trunc' ) ) {
		if ( !isBoolean( opts.trunc ) ) {
			throw new TypeError( 'zip()::invalid input argument. `trunc` option must be a boolean.  Value: `' + opts.trunc + '`.' );
		}
	} else {
		opts.trunc = true;
	}
	...
	if ( opts.hasOwnProperty( 'arrays' ) && !isBoolean( opts.arrays ) ) {
		throw new TypeError( 'zip()::invalid input argument. `arrays` option must be a boolean. Value: `' + opts.arrays + '`.' );
	}
